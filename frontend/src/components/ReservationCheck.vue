  ...
  methods: {
    async checkReservation() {
      if (this.name && this.phone) {
        try {
          const response = await axios.get('https://airlocker.biz/api/reservcheck/', {
            params: {
              name: this.name,
    ...
    async performAction1(item, index) {
      try {
        const response = await axios.post(`https://airlocker.biz/api/update-initial-verification/${item.id}/`);
    ...
    async performAction2(item, index) {
      try {
        const response = await axios.post(`https://airlocker.biz/api/update-final-verification/${item.id}/`);
